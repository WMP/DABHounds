...
from dabhounds.core.tui_report import show_terminal_summary, show_tui_report
...
def generate_report(
    ...
    for i, original in enumerate(input_tracks):
        ...
        track_id = (
            original.get("track_id") or f"{original['artist']} - {original['title']}"
        )
        json_data.append(
            {
    ...
    for i, original in enumerate(input_tracks):
        ...
        if match:
            lines.append(
                f"    DAB Track: {match['artist']} - {match['title']} (ID: {match['id']})"
            )
        else:
    ...
    # Show TUI or terminal summary based on config and show_tui parameter
    cfg = load_config()
    if show_tui and cfg.get("SHOW_TUI_OUTPUT", True):
        if cfg.get("TUI_FALLBACK_TO_TERMINAL", True):
            ...
            try:
                ...
                print(
                    "[DABHound] TUI not available. Set TUI_FALLBACK_TO_TERMINAL=true in config to show terminal summary."
                )
    else:
...
def append_tracks_to_report(
    ...
    if not report:
        return generate_report(
            ...
            match_results=[
                {"id": t.get("dab_track_id")} if t.get("dab_track_id") else None
                for t in new_tracks
            ],
            mode=matching_mode,
            library_name=library_name,
    ...
    for i, t in enumerate(report["tracks"], start=1):
        ...
        if t.get("dab_track_id"):
            lines.append(
                f"    DAB Track: {t['artist']} - {t['title']} (ID: {t['dab_track_id']})"
            )
        else:
            lines.append("    DAB Track: —")
